...
$st = $pdo->prepare("SELECT * FROM leads WHERE `column` = 'booked' ORDER BY updated_at DESC");
$st->execute();
$rows = $st->fetchAll();
...
foreach ($rows as $r) {
    $stage = $r['booking_stage'] ?: 'coming';
    if (!isset($byStage[$stage])) $byStage['coming'][] = $r;
...
<head>
  <meta charset="UTF-8">
  <title>Booked Status · Bali Diving CRM</title>
...
<header class="sticky top-0 bg-slate-950/70 backdrop-blur border-b border-slate-800">
  <div class="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between">
    <div class="flex items-center gap-3">
      <a href="index.php" class="h-8 w-8 rounded-xl bg-sky-500 text-slate-900 flex items-center justify-center font-bold">BD</a>
      <div>
        <h1 class="text-lg font-semibold">Booked – Status Board</h1>
        <p class="text-xs text-slate-400">Drag & drop between stages • Finish/Cancel will archive trip details</p>
      </div>
    </div>
    <a href="index.php"
    ...
    <?php foreach ($stages as $key => $label): $list = $byStage[$key]; ?>
      <div class="bg-slate-900/70 border border-slate-800 rounded-2xl overflow-hidden flex flex-col">
        ...
        <div class="p-2 space-y-2 min-h-[180px]" data-stage="<?=h($key)?>">
          <?php if (empty($list)): ?>
            <div class="text-[11px] text-slate-500 italic">Drop booked leads here…</div>
          <?php endif; ?>
          <?php foreach ($list as $row): ?>
            <div class="card" draggable="true" data-id="<?=h($row['id'])?>">
              <div class="text-sm font-medium truncate"><?= h($row['name'] ?: '(No Name)') ?></div>
              <div class="text-[11px] text-slate-400 truncate">
                <?= h($row['package'] ?: 'No package') ?>
              </div>
              <div class="text-[11px] text-slate-500">
                <?= !empty($row['dive_date']) ? 'Dive: '.h($row['dive_date']).' • ' : '' ?>
                Pax: <?= (int)($row['pax'] ?? 0) ?>
              </div>
            </div>
          <?php endforeach; ?>
...
(function(){
  ...
  $$('[data-stage]').forEach(col=>{
    col.addEventListener('dragover', e=>{
      e.preventDefault();
      col.classList.add('ring-1','ring-emerald-500');
    });
    col.addEventListener('dragleave', e=>{
      if(!col.contains(e.relatedTarget)){
        col.classList.remove('ring-1','ring-emerald-500');
      }
    });
    col.addEventListener('drop', async e=>{
      ...
      try{
        const j = await postForm({action:'set_booking_stage', id, stage});
        if(j.ok){
          toast('Stage: '+stage, true);
