<?php
// setting.php – Booking Calendar Settings (Global + Per-date) + Autosave
declare(strict_types=1);
date_default_timezone_set('Asia/Makassar');

include('../template/database/main.php'); 
// $pdo expected from main.php
...
/**
 * LOAD SETTINGS
 * - booking_globals.id=1.payload => {global, weeklyDefaults, weeklySubs}
 * - booking_date_snapshots.d.payload => per-date settings
 */
if ($action === 'load_settings') {
    ...
    try {
        // booking_globals: id INT PK=1, payload TEXT/JSON
        $st = $pdo->query("SELECT payload FROM booking_globals WHERE id = 1 LIMIT 1");
        if ($row = $st->fetch(PDO::FETCH_ASSOC)) {
            if (!empty($row['payload'])) {
                $payload = json_decode($row['payload'], true) ?: [];
                $global         = $payload['global']         ?? null;
                $weeklyDefaults = $payload['weeklyDefaults'] ?? null;
                $weeklySubs     = $payload['weeklySubs']     ?? null;
            }
        }

        // booking_date_snapshots: d DATE PK, payload JSON
        $st = $pdo->query("SELECT d, payload FROM booking_date_snapshots");
        while ($r = $st->fetch(PDO::FETCH_ASSOC)) {
            $key = $r['d'];
            $dateSettings[$key] = $r['payload'] ? json_decode($r['payload'], true) : null;
        }
...
/**
 * AUTOSAVE
 * - Receive JSON: global, weeklyDefaults, weeklySubs, dateSettings
 * - Save:
 *   · booking_globals.id=1.payload = {global, weeklyDefaults, weeklySubs}
 *   · booking_date_snapshots[d].payload = dateSettings[d]
 */
if ($action === 'autosave' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $weeklyDefaults = $data['weeklyDefaults'] ?? null;
    $weeklySubs     = $data['weeklySubs']     ?? null;
    $dateSettings   = $data['dateSettings']   ?? [];

    try {
        $pdo->beginTransaction();

        // Save booking_globals
        $payload = json_encode([
            'global'         => $global,
            'weeklyDefaults' => $weeklyDefaults,
            'weeklySubs'     => $weeklySubs,
        ...
        $st = $pdo->prepare("
            INSERT INTO booking_globals (id, payload, updated_at)
            VALUES (1, :p, NOW())
            ON DUPLICATE KEY UPDATE
              payload   = VALUES(payload),
              updated_at= VALUES(updated_at)
        ");
        $st->execute([':p' => $payload]);

        // Save booking_date_snapshots
        if (is_array($dateSettings)) {
            $st = $pdo->prepare("
                INSERT INTO booking_date_snapshots (d, payload, updated_at)
                VALUES (:d, :p, NOW())
                ON DUPLICATE KEY UPDATE
                  payload   = VALUES(payload),
                  updated_at= VALUES(updated_at)
            ...
            foreach ($dateSettings as $d => $cfg) {
                if (!$cfg) continue; // skip empty; delete logic bisa ditambah nanti
                $p = json_encode($cfg, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
                $st->execute([
                    ':d' => $d,
                    ':p' => $p,
                ]);
...
<body>
  <div class="wrap">
    ...
    <header class="flex items-center justify-between gap-3 pb-4 border-b border-[var(--border)]">
      ...
      <div class="flex items-center gap-2">
        ...
        <button class="btn-icon" aria-label="Settings" onclick="openGear()">
  ...
  <script>
    ...
    cutoffSel.addEventListener('change', e=> {
      GLOBAL_CUTOFF = e.target.value || '13:00';
      scheduleAutosave();
    });
    currencySel.addEventListener('change', e=> {
      GLOBAL_CURRENCY = e.target.value || 'USD';
      scheduleAutosave();
    });
    ...
    function scheduleAutosave(){
      ...
      autosaveTimer = setTimeout(sendAutosave, 800); // debounce
    }
    // expose so gear.php or other scripts can trigger autosave
    ...
    async function sendAutosave(){
      ...
      try{
        const res  = await fetch('setting.php?action=autosave', {
          method : 'POST',
        ...
        if(json.ok){
          ...
          console.warn('autosave error', json);
        }
      }catch(err){
    ...
    function showSaveBadge(){
      ...
      if(!el){
        ...
        el.style.background = 'rgba(8,47,73,.92)';
        el.style.color      = '#e0f2fe';
        el.style.zIndex     = '80';
        el.style.boxShadow  = '0 10px 25px rgba(15,23,42,.8)';
    ...
    async function loadInitialSettings(){
      try{
        const res  = await fetch('setting.php?action=load_settings', {cache:'no-store'});
        const json = await res.json();
        if(!json.ok) return;

        // Global
        if(json.global){
          GLOBAL_CUTOFF   = json.global.cutoff   || GLOBAL_CUTOFF;
          GLOBAL_CURRENCY = json.global.currency || GLOBAL_CURRENCY;
          USD_TO_IDR      = json.global.usd_to_idr || USD_TO_IDR;
          RATE_MODE       = json.global.rate_mode || RATE_MODE;

          cutoffSel.value    = GLOBAL_CUTOFF;
          currencySel.value  = GLOBAL_CURRENCY;
          rateModeSel.value  = RATE_MODE;
          if(USD_TO_IDR) usdToIdrInput.value = USD_TO_IDR;
        ...
        // Weekly defaults & subs
        if(json.weeklyDefaults){
          Object.assign(weeklyDefaults.snorkeling,   json.weeklyDefaults.snorkeling   || {});
          Object.assign(weeklyDefaults.try_diving,   json.weeklyDefaults.try_diving   || {});
          Object.assign(weeklyDefaults.fun_diving,   json.weeklyDefaults.fun_diving   || {});
          Object.assign(weeklyDefaults.padi_courses, json.weeklyDefaults.padi_courses || {});
        }
        if(json.weeklySubs){
          Object.assign(weeklySubs.snorkeling,   json.weeklySubs.snorkeling   || {});
          Object.assign(weeklySubs.try_diving,   json.weeklySubs.try_diving   || {});
          Object.assign(weeklySubs.fun_diving,   json.weeklySubs.fun_diving   || {});
          Object.assign(weeklySubs.padi_courses, json.weeklySubs.padi_courses || {});
        }

        // Per-date settings
        if(json.dateSettings && typeof json.dateSettings === 'object'){
          Object.assign(dateSettings, json.dateSettings);
        }
      }catch(err){
    ...
    function openOff(d){
      ...
      document.getElementById('max-snorkeling').oninput = e=>{ S.max.snorkeling = clampInt(e.target.value,0); scheduleAutosave(); };
      document.getElementById('max-try').oninput        = e=>{ S.max.try_diving = clampInt(e.target.value,0); scheduleAutosave(); };
      document.getElementById('max-fun').oninput        = e=>{ S.max.fun_diving = clampInt(e.target.value,0); scheduleAutosave(); };
      document.getElementById('max-padi').oninput       = e=>{ S.max.padi_courses = clampInt(e.target.value,0); scheduleAutosave(); };
    ...
    scopeWeekdayBtn.onclick = ()=>{
      ...
      if(pendingScope.type==='svc'){
        ...
        if(!weeklySubs[pendingScope.svc][wd]){
          weeklySubs[pendingScope.svc][wd] = {};
        }
      ...
      scopeTodayBtn.onclick(); // reflect today
      draw();
      scheduleAutosave();
    };

    /* ---------- INIT + GLOBAL AUTOSAVE LISTENERS ---------- */
    document.addEventListener('DOMContentLoaded', async ()=>{
      await loadInitialSettings();
      draw();

      // ANY change on inputs/selects/textareas anywhere in page => autosave
      ...
      // For typing in text/number fields, autosave while user edits (debounced)
      document.body.addEventListener('input', (e)=>{
        const t = e.target;
        if (t && (t.matches('input[type="text"]') || t.matches('input[type="number"]') || t.matches('textarea'))) {
          scheduleAutosave();
        }
      });
    });
