...
if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
    $errors[] = 'This page should be accessed from a booking page.';
} else {
    // Validasi dasar (dua step sama-sama butuh)
    ...
    // Cari lead berdasar email sekali saja
    if (!$errors) {
        try {
            if ($hasPin) {
                $st = $pdo->prepare("
    ...
    // Kalau ini confirm step, validasi full & simpan
    if (!$errors && $isConfirm) {
        if ($fullName === '') $errors[] = 'Please fill your name.';
        if ($phone === '')    $errors[] = 'Please fill your phone / WhatsApp.';
        if ($pax <= 0)        $errors[] = 'Number of participants must be at least 1.';

        // Verifikasi phone / pin kalau existing lead
        if ($existingLead) {
            if ($secret === '') {
        ...
        if (!$errors) {
            ...
            try {
                ...
                try {
                    ...
                    // kalau trip_history tidak ada, di-skip saja
                }
...
// Kalau belum confirm atau ada error -> tetap phase 'review'
if (!$success) {
    $phase = 'review';
}
?>
  ...
  <div class="max-w-3xl mx-auto px-4 py-6">
    ...
    <?php if ($phase === 'done' && !$errors): ?>
      ...
      <section class="rounded-xl border border-slate-800 bg-slate-900/60 p-4 text-sm space-y-2">
        ...
        <p><span class="text-slate-400">Email:</span> <?=htmlspecialchars($email, ENT_QUOTES, 'UTF-8')?></p>
        <p><span class="text-slate-400">Phone:</span> <?=htmlspecialchars(normalize_phone($phone), ENT_QUOTES, 'UTF-8')?></p>
        ...
        <p><span class="text-slate-400">Base price (per person):</span> $<?=number_format($baseUsd,0)?> (approx IDR <?=number_format($baseUsd*$USD_TO_IDR,0,',','.')?>)</p>
      ...
      <div class="mt-4 text-xs text-slate-500">
        You can safely close this page. If you don’t hear from us within a reasonable time, please reach out via WhatsApp on our website.
      </div>
    <?php else: ?>
      <section class="rounded-xl border border-slate-800 bg-slate-900/60 p-4 mb-6">
        <h2 class="text-lg font-semibold mb-3">Trip overview</h2>
        <div class="grid md:grid-cols-2 gap-3 text-sm">
          ...
          <div>
            ...
            <p>
              <?php if ($existingLead): ?>
                <span class="text-emerald-400">We found your previous booking data with this email.</span>
              <?php else: ?>
                <span class="text-sky-400">First time booking with this email – we’ll remember your details for next time.</span>
              <?php endif; ?>
            </p>
          </div>
        </div>
      ...
      <form method="post" class="space-y-4">
        ...
        <section class="rounded-xl border border-slate-800 bg-slate-900/60 p-4">
          ...
          <div class="space-y-3 text-sm">
            <div>
              ...
              <input type="text" name="full_name" value="<?=htmlspecialchars($fullName, ENT_QUOTES, 'UTF-8')?>"
            ...
            <div>
              ...
              <input type="text" name="country" value="<?=htmlspecialchars($country, ENT_QUOTES, 'UTF-8')?>"
                     class="w-full rounded-md border border-slate-700 bg-slate-950 px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-sky-500">
            </div>
            <div>
              <label class="block text-slate-300 mb-1">Number of participants</label>
              <input type="number" min="1" step="1" name="pax" value="<?=$pax > 0 ? $pax : 2?>"
                     class="w-full rounded-md border border-slate-700 bg-slate-950 px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-sky-500">
            </div>
          </div>
        </section>
        ...
        <section class="rounded-xl border border-slate-800 bg-slate-900/60 p-4">
          ...
          <textarea name="notes" rows="3"
                    class="w-full rounded-md border border-slate-700 bg-slate-950 px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-sky-500"
                    placeholder="Flight times, hotel area, special requests, non-swimmers, kids, etc."><?=htmlspecialchars($notes, ENT_QUOTES, 'UTF-8')?></textarea>
        </section>

        <?php if ($existingLead): ?>
          <section class="rounded-xl border border-amber-500/40 bg-amber-500/10 p-4">
            <h3 class="text-base font-semibold mb-2">Confirm it’s really you</h3>
            <p class="text-xs text-amber-100 mb-2">
              We recognise your email from previous bookings. To protect your data, please type the phone number you used before
              (<?=$maskedPhone ? 'ends with '.$maskedPhone : 'the same number you used previously'?>).
              Format is flexible: <code>08…</code>, <code>62…</code>, or <code>+62…</code>.
            </p>
            <?php if ($hasPin): ?>
            ...
            <?php endif; ?>
            <input type="text" name="verify_secret" value="<?=htmlspecialchars($secret, ENT_QUOTES, 'UTF-8')?>"
